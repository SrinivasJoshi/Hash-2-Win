...
const Puzzle = () => {
	...
	const { config: submitSolutionConfig } = usePrepareContractWrite({
	...
	const { config: answerRevealConfig } = usePrepareContractWrite({
		address: CONTRACT_ADDRESS,
		abi: ABI,
		...
		<section className='bg-purple font-montserrat flex flex-col items-center min-h-screen'>
			...
			{currentTime < puzzle.guessDeadline && (
				<button
					...
					disabled={loading}
					className='mt-10 cursor-pointer bg-orange font-bold text-purple px-3 py-2 rounded-3xl'>
					Commit Solution
				</button>
			)}
			{currentTime > puzzle.guessDeadline &&
